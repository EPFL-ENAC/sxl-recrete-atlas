...
const projectsCount = computed<Record<string, number>>(() =>
  props.projects
    ...
    .reduce(
      (acc, country) => ((acc[country] = (acc[country] || 0) + 1), acc),
...
const countriesISOAlphanumericOrder = projectsStore.countries.value.sort((a, b) =>
  a.localeCompare(b)
) as string[]
const option = computed(() => {
  ...
  const countriesISO = Object.keys(projectsCount.value).sort(
    (a, b) => projectsCount.value[b] - projectsCount.value[a]
  ) as string[]
